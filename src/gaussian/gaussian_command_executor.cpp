...
#include "gaussian_job_checker.h"
#include "gaussian_high_level_energy.h"
#include <iostream>
#include <fstream>
#include <chrono>
#include <memory>
...
int execute_extract_command(const cck::core::CommandContext& context) {
    ...
    try {
        ...
        processAndOutputResults(
            ...
            context.quiet,
            context.format,
            false, // use_input_temp - default to false
...
int execute_check_done_command(const cck::core::CommandContext& context) {
    ...
    try {
        // Find log files
        std::vector<std::string> log_files = findLogFiles(context.extension, context.max_file_size);
